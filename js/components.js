...
var pow = zero
var add = zero
function pointMult(requirement, boost, mult) {
	if (requirement) times = mult.times(boost)
	return times
}
function pointPow(requirement, boost, mult) {
	if (requirement) pow = mult.pow(boost)
	return pow
}

function pointAdd(requirement, boost, mult) {
	if (requirement) add = mult.add(boost)
	return add
}

function pointMult(mult) {
	mult = add.pow(pow.add(1)).add(add)
}

function totalBodyStat(layerID) {
	let desc = "<h3>You have total ×<em>"+ format(tmp[layerID].gainMult)+ "</em> boost for " +tmp[layerID].resource+ " with total layer boost below: </h3><br>"
	desc = desc + "<br><br>Total: ×"+ format(eval(`${layerID}g`))+ " ,+^"+ format(eval(`${layerID}p`))+ " and +"+format(ha)+ " | Total: ×"+format(tmp.h.gainMult);
	return desc
}*/
